...
class PurifierDriver extends homey_1.default.Driver {
    async onInit() {
        ...
        try {
            ...
            devicesArray.forEach(device => {
                ...
                if (savedRemotes && savedRemotes.length > 0) {
                    const ACRemotes = savedRemotes.filter(remote => remote.Type === '05');
                    airPurifiers.push(...ACRemotes);
                }
            });
            airPurifiers.forEach(item => {
                pairingList.push({
